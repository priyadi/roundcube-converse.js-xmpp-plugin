...
function rcmail_converse_init(converse, args)
{
    ...
    converse.on('onReady', function(e){
        if (!args.sid && e.target.bare_jid)
            rcmail.http_post('plugin.converse_bind', { jid:e.target.bare_jid, sid:converse.getSID() });
    });

    // store last RID for continuing session after page reload
    $(window).bind('unload', function(){
        if (window.localStorage && window.rcmail && converse.getSID()) {
            rcmail.local_storage_set_item('converse.rid', converse.getRID());
        }
    });
}
